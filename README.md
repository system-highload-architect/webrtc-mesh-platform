...
* **[RU]** Видеотрафик циркулирует строго **напрямую между браузерами по зашифрованным DTLS-SRTP туннелям (Mesh)**, освобождая сервер от нагрузки на транскодирование. На уровне сигнального протокола SDP поддерживается:
  * **Organizer Speaker Allocation (Управляемый Режим Спикера)**: Исключает хаотичный VAD-джиттер. Владелец конференции (Давид) имеет эксклюзивное право кликом назначать активного докладчика, синхронно разворачивая его видеоплитку/демонстрацию экрана на 73% площади монитора у всех участников Mesh-сессии одновременно.
